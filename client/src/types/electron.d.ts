...
declare global {
  ...
  interface ProxyState {
    enabled: boolean;
    mode: 'manual' | 'system' | 'cit';
    manualHost: string;
    manualPort: string;
    // Логин и пароль от корпоративного прокси — относятся к самому серверу,
    // а не к конкретному способу его поиска, поэтому применяются что в
    // режиме «Системный», что в «ЦИТ» (см. main.js). Сам пароль наружу из
    // main-процесса никогда не отдаётся — только факт, что он сохранён.
    citUsername: string;
    citPasswordSet: boolean;
    citReachable: boolean;
    // Диагностика последней попытки логина на прокси — единственный способ
    // отличить «неверные данные» от «прокси недоступен» и от «прокси не
    // спрашивает авторизацию», когда сам Chromium текста ошибки не даёт.
    citAuthStatus: 'no-credentials' | 'pending' | 'rejected' | null;
  }
}
