import { CalendarLayer, CalendarOccurrence, EventColor, LayerId } from './types';
...
export function colorOfLayer(id: LayerId): EventColor | 'birthday' {
...
function labelOfLayer(id: LayerId, spaceNames: Record<number, string>): string {
...
export function describeLayers(
  ...
  return ids.map((id) => ({
    id,
    label: labelOfLayer(id, spaceNames),
    color: colorOfLayer(id),
