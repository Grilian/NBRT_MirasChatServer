...
} from '@/features/emoji/customEmoji';
import {
          ...
          {emojiOpen && (
            <div className="status-emoji-popover" ref={emojiPopoverRef}>
              ...
              <EmojiPicker
                embedded
                onPick={(picked) => {
                  // Сохраняем ТО ЖЕ, что уходит в сообщение: сам символ
                  // (`token`), а `:name:` — только для смайлика без
                  // unicode-ключа, у которого символа нет.
                  //
                  // Раньше здесь всегда писался `:name:`. Этот вид кода сняли
                  // в первой волне вместе с картиночными смайликами, и
                  // отрисовщик его больше не понимает — статус показывал
                  // голый текст вместо значка. На проде смайликов без
                  // unicode-ключа нет вовсе (3770 из 3770 с ключом), так что
                  // вторая ветка — страховка, а не рабочий путь.
                  const nextEmoji = typeof picked === 'string'
                    ? picked
                    : (picked.token || `:${picked.name}:`);
                  setEmoji(nextEmoji);
