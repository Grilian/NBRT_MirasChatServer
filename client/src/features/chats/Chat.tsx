...
import ForwardModal, { ForwardPreviewItem, ForwardTarget } from './ForwardModal';
...
import type { TracesTab } from '@/features/traces/TracesSection';
...
import { StickerCatalog, buildStickerCatalog } from '@/features/stickers/stickerCatalog';
import { FILE_MAX_BYTES, FILE_TOO_LARGE_MESSAGE } from '@/shared/lib/fileLimits';
import { invalidateStickerPackCache } from '@/features/stickers/StickerPicker';
import {
...
} from '@/shared/platform/mobileNotify';
import { initMobilePush, unregisterMobilePush, dismissAllPushNotifications } from '@/shared/platform/mobilePush';
import { closeMobileInputSurface, watchMobileKeyboard, hideMobileKeyboard } from '@/shared/platform/mobileKeyboard';
import {
  ensureDesktopNotificationPermission,
  showDesktopNotification,
  dismissDesktopNotification,
  dismissAllDesktopNotifications
} from '@/shared/platform/desktopNotify';
import { playIncomingSound, primeNotificationSound } from '@/shared/lib/sound';
import {
  NotificationPrefs,
...
const Chat: React.FC = () => {
  ...
  /**
   * Открытая вкладка раздела «Следы».
   *
   * Дневник живёт на общем механизме переписки, поэтому вкладка не просто
   * переключает разметку: на ней активным чатом становится self_<id>, и вся
  ...
  const forwardTo = (ids: number[], targetChatId: string) => {
