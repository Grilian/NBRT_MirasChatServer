...
import WebDownloadLinks from '@/features/settings/WebDownloadLinks';
import { resolveUploadUrl } from '@/shared/lib/uploads';
import { useEdgeFeedback } from '@/shared/hooks/useEdgeFeedback';
import IosInstallHint from '@/features/settings/IosInstallHint';
...
const ChatList: React.FC<ChatListProps> = ({
  ...
  // Заголовок «Быстрый доступ» уже нарисован над строкой веток, поэтому строка
  // личного чата, приходящая с тем же ярлыком, свой не повторяет. Если веток
  // нет (фильтр не «Все»), заголовок нарисует сама строка личного чата.
      ...
      >
        ...
        {filtered.map((chat) => {
          ...
          const previewBody = last
            ...
            // Пустой личный чат объясняет себя сам: название задаёт
            // организация, и «Облако» или «Архив» человеку, который видит их
            // впервые, не говорят ничего. Как только там появится первое
            // сообщение, подпись сменится на его превью.
                ...
                <div className="row-body">
                  <div className="row-top">
                    <div className="row-name">
                      {/* title — на длинные названия: в строке они обрезаются
                          намеренно (список сканируют, и высота строк должна
                          быть одинаковой), и подсказка по наведению остаётся
                          единственным способом дочитать имя, не открывая чат. */}
