...
export interface Message {
  ...
  /**
   * Следы и пересылки — РАЗНЫЕ счётчики с разными источниками (traces.md).
   * Считаются на первоисточнике, поэтому на копии оба нулевые: лапка с числом
   * живёт только там, где сообщение появилось.
   */
  trace_count?: number;
  forward_count?: number;
  /** Наследил ли этот объект сам зритель — считается под конкретного человека. */
...
export interface HistoryPage {
  messages: Message[];
  /**
   * Есть ли ещё история в эту сторону. Поля НЕОБЯЗАТЕЛЬНЫЕ, и это не небрежность:
   * страница вверх ничего не знает про низ ленты, страница вниз — про верх.
   * Отсутствие поля значит «ответ про эту сторону ничего не говорит, оставьте
   * своё значение», а не «дальше пусто».
   */
  hasMoreUp?: boolean;
  hasMoreDown?: boolean;
}
