...
// Миграция: убранный из панели выбора смайлик. Строку картиночного элемента
// нельзя удалять физически — код :name: уже уехал в тексты отправленных
// сообщений, и без строки их стало бы не по чему отрисовать. `retired` убирает
// смайлик из панели выбора, оставляя старую переписку нетронутой; имя при этом
// остаётся занятым уникальным индексом, иначе то же имя можно было бы выдать
// другой картинке и задним числом переписать смысл чужих сообщений.
try {
