const db = require('../db');
const { isParticipant, participantsForChatId, parseAdminChatId, DIARY_NAME } = require('./chatParticipants');
...
function chatMeta(chatId, userId) {
  ...
  if (/^self_\d+$/.test(String(chatId))) return { name: DIARY_NAME, kind: 'self', avatar_path: null };
