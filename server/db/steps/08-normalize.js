...
module.exports = (db) => {
  // ===== Разовая уборка: названия личного чата больше не существует =====
  //
  // Оно было настройкой организации, пока смысл чата был размытым — «место,
  // куда складывают». Складывание чужого целиком ушло к Следам, и осталось
  // одно значение: личные записи, вкладка «Дневник». Имя стало константой
  // (DIARY_NAME), а настройка — мусором, который читать уже некому.
  {
    const removed = db.prepare("DELETE FROM app_settings WHERE key = 'self_chat_name'").run();
    if (removed.changes) console.log('[дневник] настройка названия личного чата снята, имя теперь постоянное');
  }
