// Разовые нормализации сохранённых значений.
//
// Шаг схемы. Порядок шагов задан в ../index.js и значение имеет: более
// поздние опираются на таблицы, заведённые более ранними.
module.exports = (db) => {
  // ===== Разовая нормализация: имя «Следы» возвращается механизму Следов =====
  //
  // Личный чат назывался «Следы» как заглушка (см. appSettings). Смены
  // умолчания на «Избранное» НЕДОСТАТОЧНО: getSelfChatName берёт умолчание
  // только при пустой настройке, а на проде в app_settings лежит явно
  // записанное слово — панель управления сохраняла его при каждом нажатии.
  // Без этой чистки в списке чатов и в разделе «Следы» стояли бы два разных
  // «Следа» одновременно.
  //
  // Сносим ТОЛЬКО ровно это слово: если организация успела назвать чат
  // по-своему («Облако», «Архив»), её выбор — не заглушка, и трогать его
  // нельзя.
  {
    const removed = db.prepare(
      "DELETE FROM app_settings WHERE key = 'self_chat_name' AND TRIM(value) = 'Следы'"
    ).run();
    if (removed.changes) console.log('[личный чат] имя «Следы» освобождено, название вернулось к умолчанию');
  }
